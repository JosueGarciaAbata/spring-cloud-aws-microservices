package com.josue.gateway_server.filters.factory;
...
public class SampleCookieGatewayFilterFactory extends AbstractGatewayFilterFactory<SampleCookieGatewayFilterFactory.ConfigurationCookie> {
    ...
    public GatewayFilter apply(ConfigurationCookie config) {
        ...
        return new OrderedGatewayFilter((exchange, chain) -> {
            logger.info("SampleCookie, pre-filter: " + config.message);

            return chain.filter(exchange)
                    .then(Mono.fromRunnable(() -> {
                        Optional.ofNullable(config.value).ifPresent(cookieValue -> {
                            ResponseCookie cookie = ResponseCookie
                                    .from(config.name, cookieValue)
                                    .build();

                            exchange.getResponse().addCookie(cookie);
                        });
                        logger.info("SampleCookie, post-filter: " + config.message);
                        }
                    ));
        }, 100);
    }
